locals {
  ...
  stack_api_image      = "ghcr.io/stack-auth/stack:${var.stack_image_tag}"
  stack_dashboard_image = "ghcr.io/stack-auth/stack-dashboard:${var.stack_image_tag}"
}
...
resource "local_file" "stack_auth_compose" {
  ...
  content  = templatefile("${path.module}/templates/docker-compose.stack-auth.yml.tftpl", {
    ...
    api_image       = local.stack_api_image
    dashboard_image = local.stack_dashboard_image
  })
}
