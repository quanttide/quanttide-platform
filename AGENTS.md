...
| cli-devops | `qtcloud-devops` — 发布管理 CLI | 安装：`uv tool install qtcloud-devops-cli` |
| cli-knowl | `qtcloud-knowl` — 知识抽取 CLI | 安装：`uv tool install qtcloud-knowl-cli` |
...
两个 CLI 工具已安装到系统（`~/.local/bin/`），可直接使用。
...
# 预检查（不执行）
qtcloud-devops release --version cli/v0.1.0 --changelog src/cli/CHANGELOG.md --dry-run

# 执行发布
qtcloud-devops release --version cli/v0.1.0 --changelog src/cli/CHANGELOG.md -y
```

支持 scope 前缀版本号（`cli/vX.Y.Z`、`python/vX.Y.Z`），非 GitHub 仓库暂不支持。
