...
子模块使用 scoped tag（如 `cli/v0.1.0`、`python/v0.1.0`）。

进入子模块内包含 `pyproject.toml` 和 `CHANGELOG.md` 的目录执行（这样无需 `--changelog` 参数）：

```bash
# 1. 进入子模块的包目录（pyproject.toml 和 CHANGELOG.md 所在目录）
cd <子模块路径>/<包目录>      # 如 apps/qtcloud-devops/src/cli
